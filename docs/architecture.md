...
- **ares-output** — tables, JSON/NDJSON, INTEL (ASN/CDN/PTR), diff, SQLite run store + living **workspace** upsert; findings taxonomy (`classify` → rule id/CWE/category) feeding **SARIF 2.1.0** export and enriched JSON
