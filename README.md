...
ares report export --format sarif --out ares.sarif   # SARIF 2.1.0 (GitHub code scanning / CI)
ares report export --format json                      # findings + taxonomy (rule_id / CWE / category)
ares report metrics                                   # severity / category / CWE / rule summary
ares report metrics --workspace --format json
ares serve --bind 127.0.0.1:7420                      # JSON control plane (jobs / workspace / runs)
# non-loopback requires --token (or --allow-remote-no-auth)
# HTTPS: ares serve --tls-cert cert.pem --tls-key key.pem [--token …]
# GET /v1/openapi.json  ·  POST /v1/jobs {"kind":"scan","targets":["127.0.0.1"],"ports":"80,443"}
ares proto vault 127.0.0.1
ares proto solr 127.0.0.1
ares proto nomad 127.0.0.1
ares proto keycloak 127.0.0.1
ares proto portainer 127.0.0.1
ares proto argocd 127.0.0.1
ares proto sonarqube 127.0.0.1
