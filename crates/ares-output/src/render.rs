...
pub fn findings_to_csv_min(collector: &EventCollector, min_rank: u8) -> String {
    let mut out = String::from("severity,host,port,peers,finding\n");
...
impl Renderer {
    pub fn new(format: OutputFormat, color: bool) -> Self {
        Self {
            format,
            color,
            quiet: matches!(format, OutputFormat::Csv | OutputFormat::Markdown),
            min_finding_rank: 0,
            show_closed: false,
            show_filtered: false,
        }
    ...
    pub fn with_quiet(mut self, quiet: bool) -> Self {
        self.quiet = quiet || matches!(self.format, OutputFormat::Csv | OutputFormat::Markdown);
        self
    }
    ...
    pub fn render_summary(&self, collector: &EventCollector, graph: &AssetGraph) {
        match self.format {
            OutputFormat::Json => {
                ...
                let findings = filter_findings_collapsed(
                    collector.findings_collapsed(),
                    self.min_finding_rank,
                )
                .into_iter()
                .map(|(host, port, severity, finding, peers)| Finding {
                    host,
                    port,
                    severity,
                    finding: if peers > 1 {
