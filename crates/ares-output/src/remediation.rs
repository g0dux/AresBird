...
pub fn remediation_for(finding: &str) -> Option<&'static str> {
    ...
    if f.contains("keycloak") {
        return Some(
            "Do not expose Keycloak publicly; require HTTPS + strong admin; lock down realms.",
        );
    }
    if f.contains("portainer") {
        return Some(
            "Never expose Portainer to the internet; require auth + TLS; bind to admin nets.",
        );
    }
    if f.contains("argo cd") || f.contains("argocd") {
        return Some("Restrict Argo CD UI/API; enforce SSO/RBAC; keep off public internet.");
    }
    if f.contains("sonarqube") {
        return Some("Require auth on SonarQube; disable anonymous; network-restrict 9000.");
    }
    ...
    if f.contains("influxdb") || f.contains("influx") {
        return Some("Require token/auth; TLS; do not expose InfluxDB HTTP (8086) publicly.");
    }
    if f.contains("rethinkdb") {
        return Some("Enable auth; bind privately; firewall 28015; prefer TLS proxy.");
    }
    if f.contains("scylla") {
        return Some(
            "Do not expose Scylla REST (10000) publicly; restrict to ops network; auth at edge.",
        );
    }
    if f.contains("elastic apm") || f.contains("apm server") {
        return Some("Require APM secret token; TLS; bind to private net; block public 8200.");
    }
    if f.contains("grpc") {
        return Some("Require mTLS or auth interceptor; network-restrict gRPC listeners; disable reflection in prod.");
    }
    if f.contains("vault") {
        return Some("Never expose Vault publicly; TLS + auth; seal recovery keys offline; restrict 8200 to control plane.");
    }
    if f.contains("nomad") {
        return Some("Enable ACLs; TLS; bind Nomad HTTP API to private networks only.");
    }
    if f.contains("solr") {
        return Some(
            "Require auth on Solr admin; bind privately; disable unauthenticated /solr/admin.",
        );
    }
    if f.contains("hazelcast") {
        return Some(
            "Disable REST if unused; enable security realm; bind privately; firewall 5701.",
        );
    }
    if f.contains("opensearch") {
        return Some("Enable security plugin / auth; TLS; do not expose 9200 publicly.");
    }
